<div class="category">
    <div class="text-center mb-4">
        <h5>Profile Building</h5>
    </div>

    @if ($reports[2][1])
        <div class="ngo">
            <p>
                <b>You then moved on to the profile building section</b>, where you were able to meet several
                Non-Governmental
                Organizations (NGO)! There are a huge amount of NGOs in Indonesia that delve into various social causes,
                such as the following:
            </p>

            <ul class="list-lower-latin">
                <li>
                    Environment
                </li>
                <li>
                    Animal Welfare
                </li>
                <li>
                    Education
                </li>
            </ul>

            <p>
                Today, we spotlighted a few NGOs that center around children, education, and healthcare aligning with
                our
                company EduALL. <b>Which one did you pick?</b>
            </p>

            <table class="table-detail">
                <tr>
                    <th>Revociety</th>
                    <td>
                        <p>
                            Revociety aims to become an innovative and trusted social company in providing services both
                            in
                            terms of education and health for people who need help and enabling people to be
                            self-sufficient.
                        </p>

                        <ul class="list-lower-latin">
                            Programs:
                            <li>
                                Health services: free minor surgery
                            </li>
                            <li>
                                Health seminars: lactation counseling for mothers, preparing medical personnel
                            </li>
                            <li>
                                Youth programs: Basic Clinical Skills Program
                            </li>
                        </ul>

                        CP: dr. Vanessa (081291428874)
                        <br>
                        Social media: <a href="https://revociety.org/">https://revociety.org/</a>

                    </td>
                </tr>
                <tr>
                    <th>Rumah Sahabat</th>
                    <td>
                        <p>
                            Rumah Sahabat aims to provide assistance and protection services for every child who is a
                            survivor of cancer or other chronic diseases in children aged 0 – 17 years from
                            underprivileged
                            families from regional areas.
                        </p>

                        <ul class="list-lower-latin">
                            Programs:
                            <li>
                                <i>Rumah Singgah Sahabat</i>
                                to visit the children and participate in classes such as drawing,
                                sharing recovery stories, and many more.
                            </li>
                            <li>
                                <i>
                                    Sahabat Responsif,
                                </i>
                                a program providing emergency response and psychosocial services to children
                                post-disaster.
                            </li>
                        </ul>


                        CP: Kak Heni (081318406833)
                        <br>
                        Social media:
                        <br>
                        <a href="https://www.instagram.com/rumah_singgah_sahabat">https://www.instagram.com/rumah_singgah_sahabat
                        </a>
                        <br>
                        <a href="https://rumahsahabat.org/">https://rumahsahabat.org/</a>


                    </td>
                </tr>
                <tr>
                    <th>Anak Harapan Indonesia</th>
                    <td>
                        <p>
                            Anak Harapan Indonesia seeks to address poverty by empowering underprivileged children and
                            families through effective education and building a community. Currently, they have a
                            learning
                            centre in West Jakarta and focus on literacy for elementary and junior high school students.
                        </p>

                        <ul class="list-lower-latin">
                            Programs:
                            <li>
                                Creating and implementing curriculum for basic literacy
                            </li>
                            <li>
                                Teaching fun classes in science, arts, English, etc.
                            </li>
                            <li>
                                Holding community classes for parents and other members on topics such as healthcare,
                                sex
                                education, etc.
                            </li>
                        </ul>


                        CP:
                        <br>
                        Social media: <a href=" https://www.instagram.com/ahaindonesia">
                            https://www.instagram.com/ahaindonesia
                        </a>

                    </td>
                </tr>

            </table>

            <p>
                From these NGOs, you can see how they operate as a non-profit organization and use it as an inspiration
                for
                your personal projects.
            </p>

            <p>
                To support these causes, you can do various volunteering activities, for example:
            </p>

            <ol>
                <li>
                    Help maintain their social media accounts so they can spread more awareness
                </li>
                <li>
                    Participate as temporary staff in their monthly events
                </li>
                <li>
                    Create events where you can lend your skills, like English class, art therapy, etc.
                </li>
            </ol>
        </div>
    @endif

    @if ($reports[2][2])
        <div class="stem">
            <p>
                <b>You then moved on to the profile building section</b>,
                you were able to meet several interesting companies that specialize in Science, Technology, Engineering,
                and
                Mathematics (STEM)!
            </p>
            <p>
                <b>Which one were you interested in?</b>
            </p>

            <table class="table-detail">
                <tr>
                    <th>Inspira Academy</th>
                    <td>
                        <p>
                            Inspira Academy specializes in courses, workshops, and programs that are designed to help
                            people
                            learn about 3D printing and how to use it for creative purposes. Inspira Academy believes
                            that
                            learning how to use design software and operate a 3D printer is a skill that is in high
                            demand
                            in the workforce and that through 3D printing students learn how to think critically and
                            creatively.
                        </p>

                        <ul class="list-lower-latin">
                            Programs and services:
                            <li>
                                Inspira Factory (cloud 3D printing services)
                            </li>
                            <li>
                                Pustaka3D™ (3D repositories): share our 3D models with the world
                            </li>
                            <li>
                                Learn: doing classes in 3D-Creation, robotics (building & programming), STEAM
                            </li>
                        </ul>
                    </td>
                </tr>
                <tr>
                    <th>Makeblock</th>
                    <td>
                        <p>
                            Makeblock specializes in developing educational robots, mechanical building platforms, and
                            STEM
                            education solutions. By integrating technology into education, Makeblock seeks to prepare
                            students for the future digital world, encouraging them to become inventors, designers, and
                            creators.
                        </p>

                        <ul class="list-lower-latin">
                            Service they provide:
                            <li>
                                Easy-to-assemble robotics kits
                            </li>
                            <li>
                                Coding platforms
                            </li>
                            <li>
                                AI and IoT learning
                            </li>
                            <li>
                                Educational software that introduces users to programming, engineering, and
                                problem-solving
                                skills in a hands-on, interactive way.
                            </li>
                        </ul>

                        Social media:
                        <br>
                        <a href="https://www.instagram.com/makeblockid/">https://www.instagram.com/makeblockid/
                        </a>
                        <br>
                        <a href="https://rumahsahabat.org/">https://rumahsahabat.org/</a>


                    </td>
                </tr>

            </table>

            <br>
            <p>
                …you were able to see how you can implement a simple personal project to boost your uni profile!
            </p>
            <p>
                For example, our EduALL mentee who made a vertical hydroponic, as he plans to major in engineering.
            </p>
            <p>
                You can try it out too! <a
                    href="https://www.youtube.com/watch?v=tmpPLiECCFU">https://www.youtube.com/watch?v=tmpPLiECCFU </a>
            </p>
            <p>
                <b>Did you get any inspiration from them? </b>
            </p>
            <p>
                Some personal projects that you can implement in line with what they produce:
            </p>
            <ul class="list-lower-latin">
                <li>
                    Creating a prototype of any object you’re interested in developing with 3D printing, for example:
                    <ul class="list-lower-roman">
                        <li>
                            Making custom parts for homes and businesses such as a phone stand.
                        </li>
                        <li>
                            Creating educational materials like models of historical artifacts or scientific models.
                        </li>
                        <li>
                            Creating art such as sculptures or jewelry.
                        </li>
                    </ul>
                </li>
                <li>
                    Finding solutions and designing IoT devices to make everyday tasks more convenient, efficient, and
                    smart
                    by connecting physical objects to the internet
                    <ul class="list-lower-roman">
                        <li>
                            Smart leak detectors
                        </li>
                        <li>
                            Smart locks
                        </li>
                        <li>
                            Smart bulbs
                        </li>
                    </ul>
                </li>
                <li>
                    Developing a website or application for your small business
                </li>
            </ul>

            <p>
                Real life projects from the above examples have been implemented by our mentees, where they integrated
                STEM
                with social aspects, check them out!
            <ol>
                <li>
                    <a href="https://feeding-hands.com/">https://feeding-hands.com/</a>
                    created by <b>Ariana Budihartanto</b>: a non-profit that focuses on reducing food
                    waste. Their accomplishments included raising funds up to $5000, redistributing 12 tons of leftovers
                    from 30+ food chains to 40+ orphanages, and even creating a website and application to support their
                    operations.
                </li>
                <li>
                    <a href="https://www.prodatblog.com/">https://www.prodatblog.com/</a>
                    created by <b>Maxwell Wijaya</b>: a blog that caters to IGCSE and IB students aspiring for STEM and
                    entrepreneurship careers, offering a gamified repository of blog posts, textbooks, courses, and
                    career
                    guidance, tailored to individual ambitions and delivering a holistic educational experience.
                </li>
            </ol>
            </p>
        </div>
    @endif


</div>
